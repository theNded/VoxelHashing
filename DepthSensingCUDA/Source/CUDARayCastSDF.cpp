...
extern "C" void computeNormals(float4* d_output,
                               float4* d_input,
                               unsigned int width,
                               unsigned int height);
extern "C" void convertDepthFloatToCameraSpaceFloat4(
        float4* d_output,
        float* d_input,
        float4x4 intrinsicsInv,
        unsigned int width,
        unsigned int height,
...
void CUDARayCastSDF::create(const RayCastParams& params) {
    ...
    m_rayIntervalSplatting.OnD3D11CreateDevice(DXUTGetD3D11Device(),
...
void CUDARayCastSDF::render(const HashData& hashData,
    ...
    if (!m_params.m_useGradients) {
        computeNormals(m_data.d_normals, m_data.d_depth4, m_params.m_width,
                       m_params.m_height);
    }
...
void CUDARayCastSDF::convertToCameraSpace(const DepthCameraData& cameraData) {
    convertDepthFloatToCameraSpaceFloat4(
            m_data.d_depth4, m_data.d_depth, m_params.m_intrinsicsInverse,
            m_params.m_width, m_params.m_height, cameraData);
...
void CUDARayCastSDF::rayIntervalSplatting(const HashData& hashData,
                                          const HashParams& hashParams,
                                          const DepthCameraData& cameraData,
                                          const mat4f& lastRigidTransform) {
    if (hashParams.m_numOccupiedBlocks == 0) return;

    if (m_params.m_maxNumVertices <=
        6 * hashParams.m_numOccupiedBlocks) {  // 6 verts (2 triangles) per
                                               // block
        MLIB_EXCEPTION(
                "not enough space for vertex buffer for ray interval "
                "splatting");
    }

    m_params.m_numOccupiedSDFBlocks = hashParams.m_numOccupiedBlocks;
    m_params.m_viewMatrix =
            MatrixConversion::toCUDA(lastRigidTransform.getInverse());
    ...
    // m_rayIntervalSplatting.rayIntervalSplatting(DXUTGetD3D11DeviceContext(),
    // hashData, cameraData, m_data, m_params,
    // m_params.m_numOccupiedSDFBlocks*6);
}
