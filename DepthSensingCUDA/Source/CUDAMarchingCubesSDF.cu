...
__global__ void extractIsoSurfaceKernel(HashData hashData,
...
extern "C" void extractIsoSurfaceCUDA(const HashData& hashData,
                                      ...
                                      MarchingCubesData& data) {
    const dim3 gridSize(params.m_hashNumBuckets * params.m_hashBucketSize, 1,
                        1);
    const dim3 blockSize(params.m_sdfBlockSize, params.m_sdfBlockSize,
                         params.m_sdfBlockSize);

    extractIsoSurfaceKernel<<<gridSize, blockSize>>>(hashData, rayCastData,
                                                     data);
...
// tags all
__global__ void extractIsoSurfacePass1Kernel(HashData hashData,
                                             RayCastData rayCastData,
                                             MarchingCubesData data) {
...
extern "C" void extractIsoSurfacePass1CUDA(const HashData& hashData,
                                           ...
                                           const MarchingCubesParams& params,
                                           MarchingCubesData& data) {
    const dim3 gridSize((params.m_hashNumBuckets * params.m_hashBucketSize +
                         (T_PER_BLOCK * T_PER_BLOCK) - 1) /
                                (T_PER_BLOCK * T_PER_BLOCK),
                        1);
    const dim3 blockSize((T_PER_BLOCK * T_PER_BLOCK), 1);

    extractIsoSurfacePass1Kernel<<<gridSize, blockSize>>>(hashData, rayCastData,
                                                          data);
...
__global__ void extractIsoSurfacePass2Kernel(HashData hashData,
                                             RayCastData rayCastData,
                                             MarchingCubesData data) {
    // const HashParams& hashParams = c_hashParams;
...
extern "C" void extractIsoSurfacePass2CUDA(const HashData& hashData,
    ...
    if (numOccupiedBlocks) {
        extractIsoSurfacePass2Kernel<<<gridSize, blockSize>>>(
                hashData, rayCastData, data);
    }
#ifdef _DEBUG
    cutilSafeCall(cudaDeviceSynchronize());
    cutilCheckMsg(__FUNCTION__);
