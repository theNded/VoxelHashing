...
template <>
struct std::hash<SDFBlockDesc>
    : public std::unary_function<SDFBlockDesc, size_t> {
    size_t operator()(const SDFBlockDesc& other) const {
        ...
        const size_t res =
                ((size_t)v.x * p0) ^ ((size_t)v.y * p1) ^ ((size_t)v.z * p2);
        return res;
    }
};
...
public:
    ChunkDesc(unsigned int initialChunkListSize) {
        ...
        m_ChunkDesc = std::vector<SDFBlockDesc>();
        m_ChunkDesc.reserve(initialChunkListSize);
    }
    ...
    unsigned int getNElements() { return (unsigned int)m_SDFBlocks.size(); }

    SDFBlockDesc& getSDFBlockDesc(unsigned int i) { return m_ChunkDesc[i]; }

    SDFBlock& getSDFBlock(unsigned int i) { return m_SDFBlocks[i]; }
    ...
    bool isStreamedOut() const { return m_SDFBlocks.size() > 0; }

    std::vector<SDFBlockDesc>& getSDFBlockDescs() { return m_ChunkDesc; }

    std::vector<SDFBlock>& getSDFBlocks() { return m_SDFBlocks; }

    const std::vector<SDFBlockDesc>& getSDFBlockDescs() const {
        return m_ChunkDesc;
    }

    const std::vector<SDFBlock>& getSDFBlocks() const { return m_SDFBlocks; }
...
inline BinaryDataStream<BinaryDataBuffer, BinaryDataCompressor>& operator<<(
        BinaryDataStream<BinaryDataBuffer, BinaryDataCompressor>& s,
        const ChunkDesc& chunkDesc) {
...
extern "C" void integrateFromGlobalHashPass1CUDA(const HashParams& hashParams,
...
extern "C" void integrateFromGlobalHashPass2CUDA(
...
extern "C" void chunkToGlobalHashPass1CUDA(const HashParams& hashParams,
                                           const HashData& hashData,
                                           uint numSDFBlockDescs,
                                           uint heapCountPrev,
                                           const SDFBlockDesc* d_SDFBlockDescs,
                                           const Voxel* d_SDFBlocks);
extern "C" void chunkToGlobalHashPass2CUDA(const HashParams& hashParams,
                                           const HashData& hashData,
                                           uint numSDFBlockDescs,
                                           uint heapCountPrev,
                                           const SDFBlockDesc* d_SDFBlockDescs,
...
public:
    CUDASceneRepChunkGrid(CUDASceneRepHashSDF* sceneRepHashSDF,
                          const vec3f& voxelExtends,
                          const vec3i& gridDimensions,
                          const vec3i& minGridPos,
                          unsigned int initialChunkListSize,
                          bool streamingEnabled,
                          unsigned int streamOutParts) {
        ...
        // m_maxNumberOfSDFBlocksIntegrateFromGlobalHash =
        // GlobalAppState::get().s_hashNumSDFBlocks /
        ...
        create(voxelExtends, gridDimensions, minGridPos, initialChunkListSize,
               streamingEnabled);
    }

    ~CUDASceneRepChunkGrid() { destroy(); }
    ...
    void streamOutToCPU(const vec3f& posCamera,
                        float radius,
                        bool useParts,
                        unsigned int& nStreamedBlocks);

    void streamOutToCPUPass0GPU(const vec3f& posCamera,
                                float radius,
                                bool useParts,
                                bool multiThreaded = true);
    void streamOutToCPUPass1CPU(bool multiThreaded = true);
    void integrateInChunkGrid(const int* desc,
                              const int* block,
                              unsigned int nSDFBlocks);

    // Stream In
    void streamInToGPUAll();
    void streamInToGPUAll(const vec3f& posCamera,
                          float radius,
                          bool useParts,
                          unsigned int& nStreamedBlocks);

    void streamInToGPUChunk(const vec3i& chunkPos);
    void streamInToGPUChunkNeighborhood(const vec3i& chunkPos,
                                        int kernelRadius);
    void streamInToGPU(const vec3f& posCamera,
                       float radius,
                       bool useParts,
                       unsigned int& nStreamedBlocks);

    void streamInToGPUPass0CPU(const vec3f& posCamera,
                               float radius,
                               bool useParts,
                               bool multiThreaded = true);
    void streamInToGPUPass1GPU(bool multiThreaded = true);

    unsigned int integrateInHash(const vec3f& posCamera,
                                 float radius,
                                 bool useParts);
    ...
    void debugDump() const {
        ...
        for (unsigned int i = 0; i < m_grid.size(); i++) {
            if (m_grid[i] != NULL) {
                std::vector<SDFBlockDesc>& descs =
                        m_grid[i]->getSDFBlockDescs();
                ...
                for (unsigned int k = 0; k < descs.size(); k++) {
                    ...
                    vec3f posWorld = vec3f(pos * SDF_BLOCK_SIZE) *
                                     hashParams.m_virtualVoxelSize;
                    hashPoints.push_back(posWorld);
                    for (unsigned int l = 0; l < linearBlockSize; l++) {
                        if (blocks[k].data[l].weight > 0 &&
                            std::fabsf(blocks[k].data[l].sdf) <= thresh) {
                            vec3i posUI = SDFBlock::delinearizeVoxelIndex(l) +
                                          pos * SDF_BLOCK_SIZE;
                            vec3f posWorld = vec3f(posUI) *
                                             hashParams.m_virtualVoxelSize;
    ...
    void startMultiThreading() {
        ...
        hStreamingThread =
                CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)StreamingFunc,
                             (LPVOID)this, 0, &dwStreamingThreadID);
    ...
    void stopMultiThreading() {
        if (!s_terminateThread) {
            ...
            if (CloseHandle(hStreamingThread) == 0) {
                std::cout << "Thread Handle GPU CPU could not be closed"
                          << std::endl;
            }
    ...
    unsigned int* getBitMaskGPU() {
        MLIB_CUDA_SAFE_CALL(cudaMemcpy(d_bitMask, m_bitMask.getRawData(),
                                       m_bitMask.getByteWidth(),
    ...
    bool isChunkInSphere(const vec3i& chunk,
                         const vec3f& center,
                         float radius) const {
        ...
        //	for (int y = -1; y<=1; y+=2)	{
        //		for (int z = -1; z<=1; z+=2)	{
        //			vec3f p = vec3f(posWorld.x+x*offset.x,
        //posWorld.y+y*offset.y, posWorld.z+z*offset.z); 			float d =
        //(p-center).length();
        ...
        float chunkExt = std::max(std::max(m_voxelExtents.x, m_voxelExtents.y),
                                  m_voxelExtents.z);
    ...
    bool containsSDFBlocksChunkInRadius(const vec3i& chunk, int chunkRadius) {
        vec3i startChunk =
                vec3i(std::max(chunk.x - chunkRadius, m_minGridPos.x),
                      std::max(chunk.y - chunkRadius, m_minGridPos.y),
                      std::max(chunk.z - chunkRadius, m_minGridPos.z));
        vec3i endChunk = vec3i(std::min(chunk.x + chunkRadius, m_maxGridPos.x),
                               std::min(chunk.y + chunkRadius, m_maxGridPos.y),
                               std::min(chunk.z + chunkRadius, m_maxGridPos.z));
    ...
    void create(const vec3f& voxelExtends,
                const vec3i& gridDimensions,
                const vec3i& minGridPos,
                unsigned int initialChunkListSize,
                bool streamingEnabled) {
        ...
        m_bitMask = BitArray<unsigned int>(
                m_gridDimensions.x * m_gridDimensions.y * m_gridDimensions.z);

        MLIB_CUDA_SAFE_CALL(cudaHostAlloc(
                &h_SDFBlockDescOutput,
                sizeof(SDFBlockDesc) *
                        m_maxNumberOfSDFBlocksIntegrateFromGlobalHash,
        ...
        MLIB_CUDA_SAFE_CALL(cudaMalloc(
                &d_SDFBlockDescOutput,
                sizeof(SDFBlockDesc) *
                        m_maxNumberOfSDFBlocksIntegrateFromGlobalHash));
        MLIB_CUDA_SAFE_CALL(cudaMalloc(
                &d_SDFBlockDescInput,
    ...
    const vec3i& getMinGridPos() const { return m_minGridPos; }

    const vec3i& getMaxGridPos() const { return m_maxGridPos; }

    const vec3f& getVoxelExtends() const { return m_voxelExtents; }
    ...
    void printStatistics() const {
        ...
        std::cout << "Total number of Blocks on the CPU: " << nSDFBlocks
                  << std::endl;
    }

    // void setPositionAndRadius(const vec3f& position, float radius, bool
    // multiThreaded) { 	if (multiThreaded) {
...
#define HASH_GRID_VERSION 1
    ...
    void loadFromFile(const std::string& filename,
        ...
        if (hashGridVersion != (unsigned int)HASH_GRID_VERSION)
            throw MLIB_EXCEPTION("hashgrid versions don't match - found " +
                                 std::to_string(hashGridVersion) +
                                 " should be " +
                                 std::to_string(HASH_GRID_VERSION));

        unsigned int numOccupiedChunks = 0;
        inStream >> numOccupiedChunks;

        if (voxelExtents != m_voxelExtents)
            throw MLIB_EXCEPTION("voxel extends don't match");
        if (gridDimensions != m_gridDimensions)
            throw MLIB_EXCEPTION("grid dimensions don't match");
        if (minGridPos != m_minGridPos)
            throw MLIB_EXCEPTION("minGridPos doesn't match");
        if (maxGridPos != m_maxGridPos)
            throw MLIB_EXCEPTION("maxGridPos doesn't match");
        if (initialChunkListSize != m_initialChunkDescListSize)
            throw MLIB_EXCEPTION("initial chunkListSize doesn't match");
    ...
    const vec3f& getPosCamera() const { return s_posCamera; }

    float getRadius() const { return s_radius; }

    bool getTerminatedThread() const { return s_terminateThread; }
...
private:
    ...
    bool isValidChunk(const vec3i& chunk) const {
        if (chunk.x < m_minGridPos.x || chunk.y < m_minGridPos.y ||
            chunk.z < m_minGridPos.z)
            return false;
        if (chunk.x > m_maxGridPos.x || chunk.y > m_maxGridPos.y ||
            chunk.z > m_maxGridPos.z)
            return false;
    ...
    vec3f numberOfChunksToMeter(const vec3i& c) const {
        return vec3f(c.x * m_voxelExtents.x, c.y * m_voxelExtents.y,
                     c.z * m_voxelExtents.z);
    }

    vec3f meterToNumberOfChunks(float f) const {
        return vec3f(f / m_voxelExtents.x, f / m_voxelExtents.y,
                     f / m_voxelExtents.z);
    }

    vec3i meterToNumberOfChunksCeil(float f) const {
        return vec3i((int)ceil(f / m_voxelExtents.x),
                     (int)ceil(f / m_voxelExtents.y),
                     (int)ceil(f / m_voxelExtents.z));
    }
    ...
    vec3i delinearizeChunkIndex(unsigned int idx) {
        ...
        unsigned int y = (idx % (m_gridDimensions.x * m_gridDimensions.y)) /
                         m_gridDimensions.x;
    ...
    void initializeCriticalSection() {
        ...
        hEventInConsume = CreateEvent(NULL, FALSE, FALSE,
                                      NULL);  //! has to be TRUE if stream out
                                              //! and in calls are split !!!
    ...
    void clearSDFBlockCounter() {
        unsigned int src = 0;
        MLIB_CUDA_SAFE_CALL(cudaMemcpy(d_SDFBlockCounter, &src,
                                       sizeof(unsigned int),
                                       cudaMemcpyHostToDevice));
    }
    ...
    unsigned int getSDFBlockCounter() const {
        unsigned int dest;
        MLIB_CUDA_SAFE_CALL(cudaMemcpy(&dest, d_SDFBlockCounter,
                                       sizeof(unsigned int),
    ...
    unsigned int m_initialChunkDescListSize;  // initial size for vectors in the
                                              // ChunkDesc
