...
extern "C" void bindInputDepthColorTextures(
        const DepthCameraData& depthCameraData) {
    cutilSafeCall(cudaBindTextureToArray(depthTextureRef,
                                         depthCameraData.d_depthArray,
                                         depthCameraData.h_depthChannelDesc));
    cutilSafeCall(cudaBindTextureToArray(colorTextureRef,
                                         depthCameraData.d_colorArray,
                                         depthCameraData.h_colorChannelDesc));
...
__global__ void resetHeapKernel(HashData hashData) {
    ...
    if (idx == 0) {
        hashData.d_heapCounter[0] =
                hashParams.m_numSDFBlocks -
                1;  // points to the last element of the array
    }
...
extern "C" void resetCUDA(HashData& hashData, const HashParams& hashParams) {
    {
        ...
        const dim3 gridSize(
                (hashParams.m_numSDFBlocks + (T_PER_BLOCK * T_PER_BLOCK) - 1) /
    ...
    {
        // resetting the hash
        const dim3 gridSize((HASH_BUCKET_SIZE * hashParams.m_hashNumBuckets +
                             (T_PER_BLOCK * T_PER_BLOCK) - 1) /
                                    (T_PER_BLOCK * T_PER_BLOCK),
                            1);
        const dim3 blockSize((T_PER_BLOCK * T_PER_BLOCK), 1);
...

    {
        // resetting the mutex
        const dim3 gridSize(
                (hashParams.m_hashNumBuckets + (T_PER_BLOCK * T_PER_BLOCK) -
                 1) / (T_PER_BLOCK * T_PER_BLOCK),
                1);
        const dim3 blockSize((T_PER_BLOCK * T_PER_BLOCK), 1);
...
extern "C" void resetHashBucketMutexCUDA(HashData& hashData,
                                         const HashParams& hashParams) {
    const dim3 gridSize(
            (hashParams.m_hashNumBuckets + (T_PER_BLOCK * T_PER_BLOCK) - 1) /
                    (T_PER_BLOCK * T_PER_BLOCK),
            1);
    const dim3 blockSize((T_PER_BLOCK * T_PER_BLOCK), 1);
...
__device__ unsigned int linearizeChunkPos(const int3& chunkPos) {
    int3 p = chunkPos - c_hashParams.m_streamingMinGridPos;
    return p.z * c_hashParams.m_streamingGridDimensions.x *
                   c_hashParams.m_streamingGridDimensions.y +
           p.y * c_hashParams.m_streamingGridDimensions.x + p.x;
}

__device__ int3 worldToChunks(const float3& posWorld) {
    float3 p;
...
__device__ bool isSDFBlockStreamedOut(
        const int3& sdfBlock,
        const HashData& hashData,
        const unsigned int* d_bitMask)  // TODO MATTHIAS (-> move to HashData)
{
    float3 posWorld =
            hashData.virtualVoxelPosToWorld(hashData.SDFBlockToVirtualVoxelPos(
                    sdfBlock));  // sdfBlock is assigned to chunk by the bottom
                                 // right sample pos
...
__global__ void allocKernel(HashData hashData,
    ...
    if (x < cameraParams.m_imageWidth && y < cameraParams.m_imageHeight) {
        ...
        // if (d == MINF || d < cameraParams.m_sensorDepthWorldMin || d >
        // cameraParams.m_sensorDepthWorldMax)	return;
        ...
        float3 boundaryPos =
                hashData.SDFBlockToWorld(idCurrentVoxel +
                                         make_int3(clamp(step, 0.0, 1.0f))) -
                0.5f * hashParams.m_virtualVoxelSize;
        float3 tMax = (boundaryPos - rayMin) / rayDir;
        float3 tDelta =
                (step * SDF_BLOCK_SIZE * hashParams.m_virtualVoxelSize) /
                rayDir;
        ...
        //	if (boundaryPos[c] - rayMin[c] == 0.0f) { tMax[c] = PINF;
        // tDelta[c] = PINF; }
        //}
        if (rayDir.x == 0.0f) {
            tMax.x = PINF;
        ...
        if (boundaryPos.x - rayMin.x == 0.0f) {
            tMax.x = PINF;
            tDelta.x = PINF;
        }
        ...
        if (boundaryPos.y - rayMin.y == 0.0f) {
            tMax.y = PINF;
            tDelta.y = PINF;
        }
        ...
        if (boundaryPos.z - rayMin.z == 0.0f) {
            tMax.z = PINF;
            tDelta.z = PINF;
        }
        ...
        unsigned int g_MaxLoopIterCount =
                1024;  // TODO MATTHIAS MOVE TO GLOBAL APP STATE
#pragma unroll 1
...
extern "C" void allocCUDA(HashData& hashData,
                          const HashParams& hashParams,
                          const DepthCameraData& depthCameraData,
                          const DepthCameraParams& depthCameraParams,
                          const unsigned int* d_bitMask) {
    const dim3 gridSize(
            (depthCameraParams.m_imageWidth + T_PER_BLOCK - 1) / T_PER_BLOCK,
            (depthCameraParams.m_imageHeight + T_PER_BLOCK - 1) / T_PER_BLOCK);
...
__global__ void fillDecisionArrayKernel(HashData hashData,
    ...
    if (idx < hashParams.m_hashNumBuckets * HASH_BUCKET_SIZE) {
        ...
        if (hashData.d_hash[idx].ptr != FREE_ENTRY) {
            if (hashData.isSDFBlockInCameraFrustumApprox(
                        hashData.d_hash[idx].pos)) {
...
extern "C" void fillDecisionArrayCUDA(HashData& hashData,
                                      const HashParams& hashParams,
                                      const DepthCameraData& depthCameraData) {
    const dim3 gridSize((HASH_BUCKET_SIZE * hashParams.m_hashNumBuckets +
                         (T_PER_BLOCK * T_PER_BLOCK) - 1) /
...
__global__ void compactifyHashKernel(HashData hashData) {
    ...
    if (idx < hashParams.m_hashNumBuckets * HASH_BUCKET_SIZE) {
        if (hashData.d_hashDecision[idx] == 1) {
            hashData.d_hashCompactified[hashData.d_hashDecisionPrefix[idx] -
                                        1] = hashData.d_hash[idx];
        }
    }
}

extern "C" void compactifyHashCUDA(HashData& hashData,
                                   const HashParams& hashParams) {
    const dim3 gridSize((HASH_BUCKET_SIZE * hashParams.m_hashNumBuckets +
                         (T_PER_BLOCK * T_PER_BLOCK) - 1) /
                                (T_PER_BLOCK * T_PER_BLOCK),
...
__global__ void compactifyHashAllInOneKernel(HashData hashData) {
    ...
    if (threadIdx.x == 0 && localCounter > 0) {
        addrGlobal =
                atomicAdd(hashData.d_hashCompactifiedCounter, localCounter);
    }
    __syncthreads();
...
extern "C" unsigned int compactifyHashAllInOneCUDA(
        HashData& hashData, const HashParams& hashParams) {
    const unsigned int threadsPerBlock = COMPACTIFY_HASH_THREADS_PER_BLOCK;
    const dim3 gridSize(
            (HASH_BUCKET_SIZE * hashParams.m_hashNumBuckets + threadsPerBlock -
             1) / threadsPerBlock,
            1);
    ...
    unsigned int res = 0;
    cutilSafeCall(cudaMemcpy(&res, hashData.d_hashCompactifiedCounter,
...
inline __device__ float4 bilinearFilterColor(const float2& screenPos) {
    ...
    const int2 dir =
            sign(make_float2(screenPos.x - p00.x, screenPos.y - p00.y));
    ...
    float w0 = 0.0f;
    if (p00.x >= 0 && p00.x < imageWidth && p00.y >= 0 && p00.y < imageHeight) {
        float4 v00 = tex2D(colorTextureRef, p00.x, p00.y);
        if (v00.x != MINF) {
            s0 += (1.0f - alpha) * v00;
            w0 += (1.0f - alpha);
        }
    }
    if (p10.x >= 0 && p10.x < imageWidth && p10.y >= 0 && p10.y < imageHeight) {
        float4 v10 = tex2D(colorTextureRef, p10.x, p10.y);
        if (v10.x != MINF) {
            s0 += alpha * v10;
            w0 += alpha;
        }
    ...
    float w1 = 0.0f;
    if (p01.x >= 0 && p01.x < imageWidth && p01.y >= 0 && p01.y < imageHeight) {
        float4 v01 = tex2D(colorTextureRef, p01.x, p01.y);
        if (v01.x != MINF) {
            s1 += (1.0f - alpha) * v01;
            w1 += (1.0f - alpha);
        }
    }
    if (p11.x >= 0 && p11.x < imageWidth && p11.y >= 0 && p11.y < imageHeight) {
        float4 v11 = tex2D(colorTextureRef, p11.x, p11.y);
        if (v11.x != MINF) {
            s1 += alpha * v11;
            w1 += alpha;
        }
    ...
    if (ww > 0.0f)
        ...
        return make_float4(MINF, MINF, MINF, MINF);
}

__global__ void integrateDepthMapKernel(HashData hashData,
                                        DepthCameraData cameraData) {
    const HashParams& hashParams = c_hashParams;
    const DepthCameraParams& cameraParams = c_depthCameraParams;
    ...
    if (screenPos.x < cameraParams.m_imageWidth &&
        screenPos.y < cameraParams.m_imageHeight) {  // on screen
        ...
        if (cameraData.d_colorData) {
            ...
            // color =
            // bilinearFilterColor(cameraData.cameraToKinectScreenFloat(pf));
        }
        ...
        if (depth != MINF) {  // valid depth
            ...
            if (depth < hashParams.m_maxIntegrationDistance) {
                ...
                if (sdf > -truncation)  // && depthZeroOne >= 0.0f &&
                                        // depthZeroOne <= 1.0f) //check if in
                                        // truncation range should already be
                                        // made in depth map computation
                {
                    if (sdf >= 0.0f) {
                    ...
                    // weightUpdate = (1-depthZeroOne)*5.0f +
                    // depthZeroOne*0.05f; weightUpdate *= g_WeightSample;
                    float weightUpdate =
                            max(hashParams.m_integrationWeightSample * 1.5f *
                                        (1.0f - depthZeroOne),
                                1.0f);
                    ...
                    if (cameraData.d_colorData) {
                        // const float4& c = tex2D(colorTextureRef, screenPos.x,
                        // screenPos.y);
                        curr.color = make_uchar3(255 * color.x, 255 * color.y,
                                                 255 * color.z);
                    } else {
                        // TODO MATTHIAS make sure there is always consistent
                        // color data
                    ...
                    Voxel newVoxel;
                    // if (color.x == MINF)
                    // hashData.combineVoxelDepthOnly(hashData.d_SDFBlocks[idx],
                    // curr, newVoxel); else
                    // hashData.combineVoxel(hashData.d_SDFBlocks[idx], curr,
                    // newVoxel);
                    hashData.combineVoxel(hashData.d_SDFBlocks[idx], curr,
                                          newVoxel);
                    hashData.d_SDFBlocks[idx] = newVoxel;
                    // Voxel prev = getVoxel(g_SDFBlocksSDFUAV,
                    // g_SDFBlocksRGBWUAV, idx); Voxel newVoxel =
                    // combineVoxel(curr, prev); setVoxel(g_SDFBlocksSDFUAV,
                    // g_SDFBlocksRGBWUAV, idx, newVoxel);
                }
            }
        }
    }
...
extern "C" void integrateDepthMapCUDA(
        HashData& hashData,
        const HashParams& hashParams,
        const DepthCameraData& depthCameraData,
        const DepthCameraParams& depthCameraParams) {
    ...
    if (hashParams.m_numOccupiedBlocks >
        0) {  // this guard is important if there is no depth in the current
              // frame (i.e., no blocks were allocated)
        integrateDepthMapKernel<<<gridSize, blockSize>>>(hashData,
                                                         depthCameraData);
    }
#ifdef _DEBUG
    cutilSafeCall(cudaDeviceSynchronize());
...
extern "C" void starveVoxelsKernelCUDA(HashData& hashData,
                                       const HashParams& hashParams) {
    const unsigned int threadsPerBlock =
            SDF_BLOCK_SIZE * SDF_BLOCK_SIZE * SDF_BLOCK_SIZE;
    const dim3 gridSize(hashParams.m_numOccupiedBlocks, 1);
...
__shared__ float
        shared_MinSDF[SDF_BLOCK_SIZE * SDF_BLOCK_SIZE * SDF_BLOCK_SIZE / 2];
__shared__ uint
        shared_MaxWeight[SDF_BLOCK_SIZE * SDF_BLOCK_SIZE * SDF_BLOCK_SIZE / 2];
...
__global__ void garbageCollectIdentifyKernel(HashData hashData) {
    ...
    // if (entry.ptr == FREE_ENTRY) return; //should never happen since we did
    // compactify before const uint linBlockSize = SDF_BLOCK_SIZE *
    ...
    shared_MinSDF[threadIdx.x] =
            min(fabsf(v0.sdf), fabsf(v1.sdf));  // init shared memory
...
#pragma unroll 1
    for (uint stride = 2; stride <= blockDim.x; stride <<= 1) {
        ...
        if ((threadIdx.x & (stride - 1)) == (stride - 1)) {
            shared_MinSDF[threadIdx.x] =
                    min(shared_MinSDF[threadIdx.x - stride / 2],
                        shared_MinSDF[threadIdx.x]);
            shared_MaxWeight[threadIdx.x] =
                    max(shared_MaxWeight[threadIdx.x - stride / 2],
                        shared_MaxWeight[threadIdx.x]);
        }
    }
    ...
    if (threadIdx.x == blockDim.x - 1) {
        ...
        float t = hashData.getTruncation(
                c_depthCameraParams
                        .m_sensorDepthWorldMax);  // MATTHIAS TODO check whether
...
extern "C" void garbageCollectIdentifyCUDA(HashData& hashData,
                                           const HashParams& hashParams) {
    const unsigned int threadsPerBlock =
            SDF_BLOCK_SIZE * SDF_BLOCK_SIZE * SDF_BLOCK_SIZE / 2;
    const dim3 gridSize(hashParams.m_numOccupiedBlocks, 1);
    const dim3 blockSize(threadsPerBlock, 1);
...
__global__ void garbageCollectFreeKernel(HashData hashData) {
    ...
    if (hashIdx < c_hashParams.m_numOccupiedBlocks &&
        hashData.d_hashDecision[hashIdx] !=
                0) {  // decision to delete the hash entry

        const HashEntry& entry = hashData.d_hashCompactified[hashIdx];
        // if (entry.ptr == FREE_ENTRY) return; //should never happen since we
        // did compactify before

        if (hashData.deleteHashEntryElement(
                    entry.pos)) {  // delete hash entry from hash (and performs
                                   // heap append)
            const uint linBlockSize =
                    SDF_BLOCK_SIZE * SDF_BLOCK_SIZE * SDF_BLOCK_SIZE;

#pragma unroll 1
            for (uint i = 0; i < linBlockSize;
                 i++) {  // clear sdf block: CHECK TODO another kernel?
...
extern "C" void garbageCollectFreeCUDA(HashData& hashData,
                                       const HashParams& hashParams) {
    const unsigned int threadsPerBlock = T_PER_BLOCK * T_PER_BLOCK;
    const dim3 gridSize((hashParams.m_numOccupiedBlocks + threadsPerBlock - 1) /
                                threadsPerBlock,
                        1);
    const dim3 blockSize(threadsPerBlock, 1);
