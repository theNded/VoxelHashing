...
LONG WINAPI StreamingFunc(LPVOID lParam) {
    ...
    while (true) {
        ...
        if (!GlobalAppState::get()
                     .s_offlineProcessing) {  // Don't use the CPU-to-GPU
                                              // streaming if offline mode; it's
                                              // called in the main thread then
            bool useParts = CUDASceneRepChunkGrid::s_useParts;
            if (GlobalAppState::get().s_offlineProcessing)
                useParts = false;  // needs to overwrite for streaming back to
                                   // the GPU

            chunkGrid->streamInToGPUPass0CPU(chunkGrid->getPosCamera(),
                                             chunkGrid->getRadius(), useParts,
                                             true);
        }
...
void CUDASceneRepChunkGrid::streamOutToCPUAll() {
    ...
    while (nStreamedBlocksSum != 0) {
        ...
        for (unsigned int i = 0; i < m_streamOutParts; i++) {
            ...
            streamOutToCPU(worldToChunks(m_minGridPos - vec3i(1, 1, 1)), 0.0f,
                           s_useParts, nStreamedBlocks);
            nStreamedBlocksSum += nStreamedBlocks;
        }
    }
}

void CUDASceneRepChunkGrid::streamOutToCPU(const vec3f& posCamera,
                                           float radius,
                                           bool useParts,
...
void CUDASceneRepChunkGrid::streamOutToCPUPass0GPU(
        const vec3f& posCamera,
        float radius,
        bool useParts,
    ...
    resetHashBucketMutexCUDA(m_sceneRepHashSDF->getHashData(),
                             m_sceneRepHashSDF->getHashParams());
    clearSDFBlockCounter();

    const unsigned int hashNumBuckets =
            m_sceneRepHashSDF->getHashParams().m_hashNumBuckets;
    const unsigned int hashBucketSize =
            m_sceneRepHashSDF->getHashParams().m_hashBucketSize;

    //-------------------------------------------------------
    // Pass 1: Find all SDFBlocks that have to be transfered
    //-------------------------------------------------------

    unsigned int threadsPerPart =
            (hashNumBuckets * hashBucketSize + m_streamOutParts - 1) /
            m_streamOutParts;
    if (!useParts) threadsPerPart = hashNumBuckets * hashBucketSize;

    uint start = m_currentPart * threadsPerPart;
    integrateFromGlobalHashPass1CUDA(m_sceneRepHashSDF->getHashParams(),
                                     m_sceneRepHashSDF->getHashData(),
                                     threadsPerPart, start, radius,
                                     MatrixConversion::toCUDA(posCamera),
                                     d_SDFBlockCounter, d_SDFBlockDescOutput);
    ...
    if (nSDFBlockDescs >= m_maxNumberOfSDFBlocksIntegrateFromGlobalHash)
        throw MLIB_EXCEPTION(
                "not enough memory allocated for intermediate GPU buffer "
                "(wants to stream out more block than "
                "m_maxNumberOfSDFBlocksIntegrateFromGlobalHash)");

    if (useParts) m_currentPart = (m_currentPart + 1) % m_streamOutParts;

    if (nSDFBlockDescs != 0) {
        // std::cout << "SDFBlocks streamed out: " << nSDFBlockDescs <<
        // std::endl;
        ...
        integrateFromGlobalHashPass2CUDA(
                m_sceneRepHashSDF->getHashParams(),
                m_sceneRepHashSDF->getHashData(), threadsPerPart,
                d_SDFBlockDescOutput, (Voxel*)d_SDFBlockOutput, nSDFBlockDescs);

        MLIB_CUDA_SAFE_CALL(cudaMemcpy(
                h_SDFBlockDescOutput, d_SDFBlockDescOutput,
                sizeof(SDFBlockDesc) * nSDFBlockDescs, cudaMemcpyDeviceToHost));
        MLIB_CUDA_SAFE_CALL(cudaMemcpy(h_SDFBlockOutput, d_SDFBlockOutput,
...
void CUDASceneRepChunkGrid::streamOutToCPUPass1CPU(
        bool multiThreaded /*= true*/) {
    if (multiThreaded) {
        ...
        if (s_terminateThread)
            return;  // avoid duplicate insertions when stop multi-threading is
                     // called
    }

    if (s_nStreamdOutBlocks != 0) {
        integrateInChunkGrid((int*)h_SDFBlockDescOutput, (int*)h_SDFBlockOutput,
                             s_nStreamdOutBlocks);
    }
...
void CUDASceneRepChunkGrid::integrateInChunkGrid(const int* desc,
                                                 const int* block,
    ...
    for (unsigned int i = 0; i < nSDFBlocks; i++) {
        ...
        vec3f posWorld =
                vec3f(pos * SDF_BLOCK_SIZE) * hashParams.m_virtualVoxelSize;
        ...
        // Add element
        m_grid[index]->addSDFBlock(((const SDFBlockDesc*)desc)[i],
                                   ((const SDFBlock*)block)[i]);
...
void CUDASceneRepChunkGrid::streamInToGPUAll() {
    ...
    while (nStreamedBlocks != 0) {
        streamInToGPU(getChunkCenter(vec3i(0, 0, 0)),
                      1.1f * getGridRadiusInMeter(), s_useParts,
                      nStreamedBlocks);
    }
}

void CUDASceneRepChunkGrid::streamInToGPUAll(const vec3f& posCamera,
                                             float radius,
                                             bool useParts,
                                             unsigned int& nStreamedBlocks) {
...
void CUDASceneRepChunkGrid::streamInToGPUChunk(const vec3i& chunkPos) {
    ...
    {
        streamInToGPU(getChunkCenter(chunkPos), 1.1f * getChunkRadiusInMeter(),
                      true, nStreamedBlocks);
    }
}

void CUDASceneRepChunkGrid::streamInToGPUChunkNeighborhood(
        const vec3i& chunkPos, int kernelRadius) {
    vec3i startChunk =
            vec3i(std::max(chunkPos.x - kernelRadius, m_minGridPos.x),
                  std::max(chunkPos.y - kernelRadius, m_minGridPos.y),
                  std::max(chunkPos.z - kernelRadius, m_minGridPos.z));
    vec3i endChunk = vec3i(std::min(chunkPos.x + kernelRadius, m_maxGridPos.x),
                           std::min(chunkPos.y + kernelRadius, m_maxGridPos.y),
                           std::min(chunkPos.z + kernelRadius, m_maxGridPos.z));
...
void CUDASceneRepChunkGrid::streamInToGPU(const vec3f& posCamera,
                                          float radius,
                                          bool useParts,
                                          unsigned int& nStreamedBlocks) {
    s_posCamera = posCamera;
...
void CUDASceneRepChunkGrid::streamInToGPUPass0CPU(
        const vec3f& posCamera,
        float radius,
        bool useParts,
        bool multiThreaded /*= true*/) {
...
void CUDASceneRepChunkGrid::streamInToGPUPass1GPU(
        bool multiThreaded /*= true*/) {
    if (multiThreaded) {
    ...
    if (s_nStreamdInBlocks != 0) {
        // std::cout << "SDFBlocks streamed in: " << s_nStreamdInBlocks <<
        // std::endl;
        ...
        MLIB_CUDA_SAFE_CALL(cudaMemcpy(
                &heapCountPrev, m_sceneRepHashSDF->getHashData().d_heapCounter,
                sizeof(unsigned int), cudaMemcpyDeviceToHost));

        chunkToGlobalHashPass1CUDA(
                m_sceneRepHashSDF->getHashParams(),
                m_sceneRepHashSDF->getHashData(), s_nStreamdInBlocks,
                heapCountPrev, d_SDFBlockDescInput, (Voxel*)d_SDFBlockInput);

        //-------------------------------------------------------
        // Pass 2: Initialize corresponding SDFBlocks
        //-------------------------------------------------------

        chunkToGlobalHashPass2CUDA(
                m_sceneRepHashSDF->getHashParams(),
                m_sceneRepHashSDF->getHashData(), s_nStreamdInBlocks,
                heapCountPrev, d_SDFBlockDescInput, (Voxel*)d_SDFBlockInput);
        ...
        MLIB_CUDA_SAFE_CALL(
                cudaMemcpy(m_sceneRepHashSDF->getHashData().d_heapCounter,
                           &initialCountNew, sizeof(unsigned int),
...
unsigned int CUDASceneRepChunkGrid::integrateInHash(const vec3f& posCamera,
                                                    float radius,
                                                    bool useParts) {
    ...
    vec3i startChunk =
            vec3i(std::max(camChunk.x - chunkRadius.x, m_minGridPos.x),
                  std::max(camChunk.y - chunkRadius.y, m_minGridPos.y),
                  std::max(camChunk.z - chunkRadius.z, m_minGridPos.z));
    vec3i endChunk =
            vec3i(std::min(camChunk.x + chunkRadius.x, m_maxGridPos.x),
                  std::min(camChunk.y + chunkRadius.y, m_maxGridPos.y),
                  std::min(camChunk.z + chunkRadius.z, m_maxGridPos.z));
        ...
        for (int y = startChunk.y; y <= endChunk.y; y++) {
            for (int z = startChunk.z; z <= endChunk.z; z++) {
                ...
                if (m_grid[index] != NULL &&
                    m_grid[index]->isStreamedOut())  // As been allocated and
                                                     // has streamed out blocks
                {
                    if (isChunkInSphere(delinearizeChunkIndex(index), posCamera,
                                        radius))  // Is in camera range
                    {
                        unsigned int nBlock = m_grid[index]->getNElements();
                        if (nBlock + nSDFBlocks >
                            m_maxNumberOfSDFBlocksIntegrateFromGlobalHash) {
                            throw MLIB_EXCEPTION(
                                    "not enough memory allocated for "
                                    "intermediate GPU buffer (wants to stream "
                                    "out more block than "
                                    "m_"
                                    "maxNumberOfSDFBlocksIntegrateFromGlobalHas"
                                    "h)");
                        }
                        // Copy data to GPU
                        MLIB_CUDA_SAFE_CALL(cudaMemcpy(
                                d_SDFBlockDescInput + nSDFBlocks,
                                &(m_grid[index]->getSDFBlockDescs()[0]),
                                sizeof(SDFBlockDesc) * nBlock,
                                cudaMemcpyHostToDevice));
                        MLIB_CUDA_SAFE_CALL(
...
void CUDASceneRepChunkGrid::debugCheckForDuplicates() const {
    ...
    unsigned int numHashEntries =
            m_sceneRepHashSDF->getHashParams().m_hashBucketSize *
            m_sceneRepHashSDF->getHashParams().m_hashNumBuckets;
    HashEntry* hashCPU = new HashEntry[numHashEntries];
    MLIB_CUDA_SAFE_CALL(cudaMemcpy(
            hashCPU, m_sceneRepHashSDF->getHashData().d_hash,
            sizeof(HashEntry) * numHashEntries, cudaMemcpyDeviceToHost));
    for (unsigned int i = 0; i < numHashEntries; i++) {
        if (hashCPU[i].ptr != FREE_ENTRY) {
            SDFBlockDesc curr(hashCPU[i]);
            if (descHash.find(curr) == descHash.end())
                descHash.insert(curr);
            else
                throw MLIB_EXCEPTION(
                        "Duplicate found in streaming hash data (in hash)");
        }
    }
    ...
    for (unsigned int i = 0; i < m_grid.size(); i++) {
        if (m_grid[i] != NULL) {
            std::vector<SDFBlockDesc>& descsCopy =
                    m_grid[i]->getSDFBlockDescs();

            for (unsigned int k = 0; k < descsCopy.size(); k++) {
                if (descHash.find(descsCopy[k]) == descHash.end())
                    descHash.insert(descsCopy[k]);
                else
                    throw MLIB_EXCEPTION(
